...
def mi_reply(key, method, code, reason, body=None):
    logging.info(""" Replies to the server """)
    params = {'key': key,
              'method': method,
              'code': code,
              'reason': reason}
...
def parse_params(params):
    logging.info(""" Parses paraameters received in a call """)
    flavor = None
    extra_params = None
...
def handle_call(call, key, method, params):
    logging.info(""" Handles a SIP call """)
...
def udp_handler(data):
    logging.info(""" UDP handler of events received """)
...
async def shutdown(s, loop, event):
    logging.info(""" Called when the program is shutting down """)
...
async def async_run():
    ...
    try:
        ...
        logging.info(""" E_UA_SESSION setado => udp_handler """)
    except OpenSIPSEventException as e:
    ...
    logging.info(""" add_signal_handler => signal.SIGTERM """)
    ...
    logging.info(""" add_signal_handler => signal.SIGINT """)

    try:
        logging.info(""" await stop """)
        await stop
...
def run():
    logging.info(""" Runs the entire engine asynchronously """)
