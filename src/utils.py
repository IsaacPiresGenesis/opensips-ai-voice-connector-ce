...
class UnknownSIPUser(Exception):
    logging.info(""" Instanciou a classe """)
    logging.info(""" User is not known """)


def get_header(params, header):
    logging.info(""" Returns a specific line from headers """)
...
def get_to(params):
    logging.info(""" Returns the To line parameters """)
...
def indialog(params):
    logging.info(""" indicates whether the message is an in-dialog one """)
    if 'headers' not in params:
        return False
    to = get_to(params)
    if not to:
...
def get_user(params):
    logging.info(""" Returns the User from the SIP headers """)
...
def _dialplan_match(regex, string):
    logging.info(""" Checks if a regex matches the string """)
    pattern = re.compile(regex)
    return pattern.match(string)


def get_ai_flavor_default(user):
    logging.info(""" Returns the default algorithm for AI choosing """)
...
def get_ai_flavor(params):
    logging.info(""" Returns the AI flavor to be used """)
...
def get_ai(flavor, call, cfg):
    logging.info(""" Returns an AI object """)
